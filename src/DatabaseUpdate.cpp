    ...
    if (result)
    {
        ...
        do
        {
            ...
            if (fields)
            {
                ...
                // Must be used because the core for the Insert
                insertsql.append("1','1','35','35','1','1','").append(_WDB_VERIFIED).append("')");

                // We have reached MAX_INSERTS and data left over? Then insert send and delete!
        ...
        do
        {
            ...
            if (fields)
            {
                for (int8 i=0; i<31; ++i)
                {
                    ...
                    {
                        if (own_style)
                        {   // Boxes and fishing spots
                            if (i == 8 && (fields[i].GetUInt32() == 3 || fields[i].GetUInt32() == 25))
                            {
                                sprintf(tmp, "%u", fields[0].GetUInt32()); // entry as lootid
                                insertsql.append(tmp).append("','");
                            }
                            else
                            {
...
void DatabaseUpdate::CreateItemInsert(const char* wdbdb, const char* worlddb, Database* DB, const char* home)
{
    ...
    query.append("SELECT `entry`,`Class`,`SubClass1`,`unk0`,`Name1`,`ItemDisplayID`,`Quality`,`Flags`,"
        ...
        "`Stat10`,`Stat10Val`," // Double due to `ScalingStatDistribution`,`ScalingStatValue` in the Insert! So that it fits!
        // 48
        ...
        //int fields 3,7,8,11,12,22,23,26-47,64-93,94,98,101,102,103,120,122
        //float fields 48,49,51,52,63,121
                        ...
                        case 94: case 98: case 101: case 102: case 103: case 120: case 122:
                            {
                                ...
                                // Account bound items with varying stats!
                                if (i == 44 || i == 45)// StatType10 + StatValue10
                                {                      // StatCount
                                    if (tmpint != 0 && fields[25].GetInt32() == 0)
                                    {   // At 'ScalingStatDistribution', reset the value 'ScalingStatValue'
                        ...
                        default:
                            {
                                ...
                                ################################################################################################################
                                # Faction-"Correction" due to core! :-( UPDATE 'itemcache' SET 'reqfactionlvl ' ='0 ' WHERE'reqfaction ' = '0';#
                                ################################################################################################################*/
                                if (i == 21 && tmpuint > 0)
    ...
    if (result)
    {
        ...
        do
        {
            ...
            if (fields)
            {
                ...
                // We have reached MAX_INSERTS and then send insert and delete data left?
                if (count >= MAX_INSERTS && (count+counttotal+1) < result->GetRowCount())
                {
                    insertsql.append(";\n");
                    fputs(insertsql.c_str(), sqlfile);
                ...
                for (uint8 i=0; i<10; ++i)
                {
                    if ((docolumn && ColumnExists(columns, column1[i])) || !docolumn)
                    {
                        if (fields[7+i*2].GetUInt32() != fields[8+i*2].GetUInt32())
                        {
                            ...
                            // NO MODEL UPDATE FOR THESE ENTRIES BECAUSE INCORRECT DATA IN THE WDBS STANDING!
                ...
                for (uint8 i=0; i<9; i++)
                {
                    if ((docolumn && ColumnExists(columns, column1[i])) || !docolumn)
                    {
                        if (fields[25+i*2].GetUInt32() != fields[26+i*2].GetUInt32())
                        {
                            ...
                            // Faction-"Correction" due to core! :-(

...
IO::IO(void)
{
    ...
    cfg->dummy      = 256; // old resolution (resolution) in the map extract
    cfg->db_user    = "";
    cfg->db_pw      = "";
    ...
    cfg->wdb_db     = "wdbdata";
    cfg->wowpath    = "";
    cfg->mysqlpath  = ""; // Necessary because mysqldump under Windows, if MySQL is not in the system path is
    cfg->world_db   = "world";
...
{   // Lengths of the data set
    cfg_header->dummy.size = sizeof(cfg->dummy); // old resolution (resolution) in the map extract
    ...
    // Initial positions of the data in the file set
    cfg_header->dummy.pos  = sizeof(cfgheader); // old resolution (resolution) in the map extract
...
// CFG makes data readable again (can)
void IO::DecodeCfg()
{
...
void IO::LoadCFG()
{
    ...
    fread(&cfg->dummy, cfg_header->dummy.size, 1, cfgfile); // old resolution (resolution) in the map extract
...
#if PLATFORM == PLATFORM_WINDOWS
    // MySQL-Path (required only under Windows) to determine
...
// Look if search is available in array, and returns a pointer to it
...
// Makes a string readable again (can)
...
// Deletes \ or / or "at the end of a path
...
// Converts a string to WCHAR (UTF8)
...
void IO::GetDBCDatasAndWriteToDatabase(const char* db, const char* user, const char* pw, const char* host)
{
    ...
    // MySQL Initialize and prepare DBs
    if (!mysqlready)
    {
    ...
    // Number of all *. Dbc files to determine
    struct dirent* ent;
    if (dir != NULL)
    {
    ...
    // Column names and descriptions initialize
    FillDBCColumnNames();

    // Go over all the DBC files
    for (uint16 i=0; i<MAX_DBCS; i++)
    {
        ...
        // File has 0 bytes @ 3.0.3.9183
        if (DBCFileList[i].filename == "CharVariations.dbc") continue;

        // Special case without PRIMARY KEY!
        ...
        if (file.IsLoaded())
        {   // DBCFile has fewer or more columns
            ...
            for (uint32 j=0; j<file.GetNumRows(); j++)
            {
                ...
                // Go through all the columns (a record)
                for (int32 k=0; k<formatsize; k++)
                {
                    ...
                    // Queries single column data for a data set
                    ...
                    // Get even more records, or do we have to stop the INSERT?
                    if ((j+1) == file.GetNumRows() && (k+1) == formatsize) sqlinsert += "')";
                    else if ((k+1) == formatsize) sqlinsert += "'),";
                    else sqlinsert += "',";
                }
                // SQL structure create only 1 x can be
                structdone = true;
            }
            if (noprimarykey && !structcreated)
            {
    ...
    // Number of found DBC files is not our number!
...
void IO::GetWDBDatasAndWriteToDatabase(const char* db, const char* user, const char* pw, const char* host)
{
    ...
    // Initialize MySQL and DB prepare
    if (DB.Initialize(user, pw, NULL, host))
    {
    ...
    // WDB_MAX_FILES is defined in Defines.h with 8, because the wowcache.wdb format is not known!
            ...
            if (wdb.isLoaded)
            {
                ...
                for (uint32 j=0; j<wdb.m_records.size(); j++)
                {
                    ...
                    for (uint32 k=1; k<=nFields; k++)
                    {
                        ...
                        // We have reached MAX_QUERY_LEN nearly and data left over? Then sqlinsert send and delete!
...
// Since much too big, this block was exported (initialize column names and descriptions)
